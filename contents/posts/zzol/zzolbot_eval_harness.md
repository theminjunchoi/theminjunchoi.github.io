...
예를 들어 "응답이 느려요" 시나리오라면 — 질문은 "요즘 전체적으로 응답이 느려요. 점검해줘", 고정해둔 외부 데이터는 `redis_stream_status → {game-stream: 48213}`(백로그), 채점 기준은 "Redis game-stream 백로그를 근본원인으로 짚으면 정답, 정상이라 답하면 오답"이다.

이때 채점 기준은 외부 데이터가 "백로그 48213개"일 때만 정답이다. 만약 평가하면서 라이브 Redis를 봐서 백로그가 이미 풀려 있었다면, "정상"이라 답한 봇이 맞는데도 오답 처리된다. **질문과 정답은 한 쌍이라, 그 순간의 외부 데이터를 고정해야 채점이 성립한다.**
